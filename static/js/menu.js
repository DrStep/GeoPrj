  ...
  if (id==prevId){
	...
	switch(id){
      ...
      $.ajax({
                    ...
                    dataType: "json",
                    data: "fields=['name','age','photo',]&user_id=10",
                    beforeSend: function() {
                    },
    ...
    case 'meetings':
      $('.changePoint3').css({background:'white'});
            $.ajax({
                          ...
                          dataType: "json",
                          data: "fields=['title','description','photo','status','date_time']&user_id=10",
                          beforeSend: function() {
                          },
                          success: function(res) {
                              ...
                              var result='';
                              for(i=0;i<res.length;i++) {
                                     result+="   <div class='one_meeting'> <div class='meeting_block'> <div id='meeting_square'></div> <div id='descriptionBlock'> <span id='meeting_font'>Поход в парк</span> <br> <span>Дата начала мероприятия:</span> <br> <span>Время начала:</span> <br> <span>Описание:</span> <br><br> </div> <div id='buttonBlock'> <button class='button_meeting'>Присоединиться</button> <button class='button_meeting'>Не интересует</button><button class='button_meeting'>Напоминание</button></div> </div> </div> ";
                              }
                              return result;
                              })
                           },
...
$('#m7 a').css({color:'white'});
             $.ajax({
                          ...
                          url: "http://localhost:8090/api/users.dialogs",
                                 dataType: "json",
                                 data: "fields=['name','latitude','longitude']&user_id=10",
                                 beforeSend: function() {
                                 },
                                 success: function(res) {
